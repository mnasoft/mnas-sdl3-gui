...
(defmethod handle-keyboard-event ((widget <list-box>) (ev widget-keyboard-input))
  (let* ((key (widget-keyboard-input-key ev))
    ...
    (cond
      ((eq key :up)
       (when (> (selected-index widget) 0)
         (decf (selected-index widget))
         (ensure-list-box-selection-visible widget)
         (update-<widget>-value widget (nth (selected-index widget)
                                            (list-box-items widget))))
       t)
      ((eq key :down)
       (when (< (1+ (selected-index widget))
                (length (list-box-items widget)))
         (incf (selected-index widget))
         (ensure-list-box-selection-visible widget)
         (update-<widget>-value widget (nth (selected-index widget)
                                            (list-box-items widget))))
       t)
      ((eq key :pageup)
       (when (> (selected-index widget) 0)
         (setf (selected-index widget)
               (max 0 (- (selected-index widget) visible-count)))
         (ensure-list-box-selection-visible widget)
         (update-<widget>-value widget (nth (selected-index widget)
                                            (list-box-items widget))))
       t)
       ...
       (when (plusp (length (list-box-items widget)))
         (setf (selected-index widget)
               (min (1- (length (list-box-items widget)))
                    (+ (selected-index widget) visible-count)))
         (ensure-list-box-selection-visible widget)
         (update-<widget>-value widget (nth (selected-index widget)
                                            (list-box-items widget))))
       t)
      ...
      ((eq key :end)
       (when (plusp (length (list-box-items widget)))
         (setf (selected-index widget)
               (1- (length (list-box-items widget))))
         (ensure-list-box-selection-visible widget)
         (update-<widget>-value widget (nth (selected-index widget)
                                            (list-box-items widget))))
       t)
...
(defmethod handle-keyboard-event ((widget <combo-box>) (ev widget-keyboard-input))
  (let* ((key (widget-keyboard-input-key ev))
    ...
    (cond
      ((eq key :return)
       (if (expanded-p widget)
           (progn
             (sync-combo-box-expanded-state widget nil)
             (update-<widget>-value widget (selected-item widget))
             t)
           (activate-widget widget)))
      ((eq key :space)
       (activate-widget widget))
      ((eq key :up)
       ...
       t)
      ((eq key :down)
       (when (< (1+ (selected-index widget)) item-count)
         (incf (selected-index widget))
         (ensure-combo-box-selection-visible widget)
         (update-<widget>-value widget (selected-item widget)))
       t)
