...
(defparameter *combo-box-02-layer-manager* nil)
...
(sdl3:def-app-init combo-box-02-demo-init (argc argv)
  ...
  (setf *combo-box-02-layer-manager*
    ...
    (if (not ok)
        ...
        (progn
          (setf *combo-box-02-window* window
               ...
               *combo-box-02-layer-manager*
               *combo-box-02-window-id*
               :main
               :open-p t)
  ...
  (when *combo-box-02-toolbar*
     ...
     *combo-box-02-renderer*
     ...
     (- +combo-box-02-window-height+ +combo-box-02-toolbar-height+)))
      (loop for widget in (mnas-sdl3-gui/widgets:widgets-in-render-order *combo-box-02-widgets*)
        do (mnas-sdl3-gui/widgets:render *combo-box-02-renderer* widget mnas-sdl3-gui/widgets:*widget-style*))
        ...
        (typecase ev
          ...
          (sdl3:window-event
           (let* ((window-id (slot-value ev 'sdl3:%window-id))
                  (main-id (and *combo-box-02-window* (sdl3:get-window-id *combo-box-02-window*)))
          ...
          (sdl3:mouse-button-event
           ...
           (sdl3:mouse-wheel-event
              (let* ((win-id (slot-value ev 'sdl3:%window-id))
                     ...
                     (mx (or (ignore-errors (round (slot-value ev 'sdl3:%mouse-x)))
                             (round (slot-value ev 'sdl3:%x))))
                     (my (or (ignore-errors (round (slot-value ev 'sdl3:%mouse-y)))
